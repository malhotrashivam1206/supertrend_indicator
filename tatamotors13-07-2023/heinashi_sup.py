...
def calculate_supertrend(data, atr_period=12, factor=2.0):
    ...
    # Calculate Heikin Ashi open, high, low, close
    ha_open = (close.shift(1) + close.shift()) / 2
    ha_high = high.combine(ha_open, max)
    ha_low = low.combine(ha_open, min)
    ...
    for i in range(1, len(data)):
        if ha_close.iloc[i] > supertrend.iloc[i - 1]:
            supertrend.iloc[i] = data['lower_band'].iloc[i] if ha_close.iloc[i - 1] > supertrend.iloc[i - 1] else data['upper_band'].iloc[i]
            direction.iloc[i] = 1
        else:
            supertrend.iloc[i] = data['upper_band'].iloc[i] if ha_close.iloc[i - 1] < supertrend.iloc[i - 1] else data['lower_band'].iloc[i]
            direction.iloc[i] = -1

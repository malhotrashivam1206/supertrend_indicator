...
data_file_path = "ohlc_datasupphase5-1.csv"
graph_file_path = "candlest_graphsup5-1.html"
...
def calculate_supertrend(data, atr_period=10, factor=3.0):
    ...
    for i in range(1, len(data)):
        if direction.iloc[i - 1] == 1 and close.iloc[i] < supertrend.iloc[i - 1]:
            supertrend.iloc[i] = data['upper_band'].iloc[i]
            direction.iloc[i] = -1
        elif direction.iloc[i - 1] == -1 and close.iloc[i] > supertrend.iloc[i - 1]:
...
def update_graph(n, interval, chart_type):
    ...
    if chart_type == 'heikin_ashi':
        # Calculate Heikin Ashi OHLC data
        ha_data = calculate_heikin_ashi(resampled_data)
        resampled_data = ha_data
